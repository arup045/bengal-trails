...
export function AnalyticsDashboard() {
  ...
  const { data, isLoading: loading, error: qError, refetch } = useAdminStats(range);
  const load = (r: string) => { setRange(r); };

  const error = qError?.message || '';
  if (loading) return (
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center py-20 gap-3">
      ...
      <button onClick={() => refetch()} className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-xl font-poppins text-sm">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        <StatCard icon={Users}     label="Total users"        value={stats.totalUsers}        sub={`+${stats.newUsersInRange} this period`} color="bg-purple-500" />
        ...
        <StatCard icon={TrendingUp}label="Enquiries received" value={stats.totalEnquiries}    color="bg-rose-500" />
      </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={trend}>
              ...
              <XAxis dataKey="date" tick={{ fontSize: 11, fontFamily: 'Poppins' }} tickFormatter={d => d.slice(5)} />
              <YAxis tick={{ fontSize: 11, fontFamily: 'Poppins' }} />
              <Tooltip contentStyle={{ fontFamily: 'Poppins', fontSize: 12, borderRadius: 12 }} />
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={topDests} layout="vertical">
              ...
              <YAxis type="category" dataKey="name" tick={{ fontSize: 11, fontFamily: 'Poppins' }} width={110} />
              <Tooltip contentStyle={{ fontFamily: 'Poppins', fontSize: 12, borderRadius: 12 }} />
