    ...
    <div className="max-w-2xl mx-auto px-5 sm:px-8 py-10">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-2 bg-green-50 px-4 py-1.5 rounded-full mb-4">
          ...
          <span className="font-poppins text-sm font-medium text-green-700">Partner with Bengal Trails</span>
        </div>
          ...
          {(['type','details','review'] as Step[]).map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center font-poppins text-xs font-semibold transition-all
                ${step === s ? 'bg-green-600 text-white' : i < ['type','details','review'].indexOf(step) ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>
                {i + 1}
              </div>
        ...
        {step === 'type' && (
          <motion.div key="type" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}>
              ...
              {TYPES.map(t => {
                ...
                return (
                  <button key={t.value} onClick={() => set('businessType', t.value)}
                    ...
                    <Icon className={`w-6 h-6 mb-2 ${sel ? 'text-green-600' : 'text-gray-400'}`} />
            ...
            <button disabled={!form.businessType} onClick={() => setStep('details')}
              className="w-full mt-6 py-3.5 rounded-xl bg-green-600 hover:bg-green-700 font-poppins font-medium text-white transition-colors disabled:opacity-40 flex items-center justify-center gap-2">
          ...
          <motion.div key="details" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}} className="space-y-5">
            ...
            {[
              ...
              <div key={f.k}>
                ...
                <input type={f.type} value={(form as any)[f.k]} onChange={e => set(f.k, e.target.value)}
                  ...
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl font-poppins text-sm focus:outline-none focus:border-green-400 transition-colors" />
              </div>
            ))}
            ...
            <div>
              ...
              <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={3}
                ...
                className="w-full px-4 py-2.5 border border-gray-200 rounded-xl font-poppins text-sm focus:outline-none focus:border-green-400 transition-colors resize-none" />
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => set('experienceYrs', Math.max(0, form.experienceYrs - 1))}
                  className="w-9 h-9 rounded-full border border-gray-200 hover:border-green-400 flex items-center justify-center">−</button>
                ...
                {AREAS.map(a => (
                  <button key={a} onClick={() => toggleArr('serviceAreas', a)}
                    className={`px-3 py-1.5 rounded-full font-poppins text-xs transition-all
                      ${form.serviceAreas.includes(a) ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                    {a}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {LANGS.map(l => (
                  <button key={l} onClick={() => toggleArr('languages', l)}
                    className={`px-3 py-1.5 rounded-full font-poppins text-xs transition-all
                      ${form.languages.includes(l) ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                    {l}
                  </button>
                ))}
              </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button onClick={() => { if (!form.businessName) { toast.error('Enter your business name'); return; } setStep('review'); }}
                className="flex-1 py-3 rounded-xl bg-green-600 hover:bg-green-700 font-poppins text-sm font-medium text-white transition-colors flex items-center justify-center gap-2">
          ...
          <motion.div key="review" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={submit} disabled={loading}
                className="flex-1 py-3 rounded-xl bg-green-600 hover:bg-green-700 font-poppins text-sm font-medium text-white transition-colors disabled:opacity-60 flex items-center justify-center gap-2">
